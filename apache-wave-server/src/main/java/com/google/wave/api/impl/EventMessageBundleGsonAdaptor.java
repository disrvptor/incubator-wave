...
import com.google.wave.api.BlipImpl;
import com.google.wave.api.OperationQueue;
import com.google.wave.api.OperationQueueImpl;
import com.google.wave.api.OperationRequest;
import com.google.wave.api.BlipThread;
import com.google.wave.api.Wavelet;
import com.google.wave.api.WaveletImpl;
...
public class EventMessageBundleGsonAdaptor implements
  ...
  public EventMessageBundle createInstance(Type type) {
    return new EventMessageBundleImpl("", "http://opensocial.example.com");
  }
  ...
  public EventMessageBundle deserialize(JsonElement json, Type typeOfT,
    ...
    EventMessageBundle result = new EventMessageBundleImpl(robotAddress, rpcServerUrl);

    OperationQueue operationQueue;
    if (jsonObj.has(PROXYING_FOR_TAG)) {
      result.setProxyingFor(jsonObj.get(PROXYING_FOR_TAG).getAsString());
      operationQueue = new OperationQueueImpl(new ArrayList<OperationRequest>(),
          result.getProxyingFor());
    } else {
      operationQueue = new OperationQueueImpl();
    }
    ...
    Wavelet wavelet = WaveletImpl.deserialize(operationQueue, blips, threads, waveletData);
    ...
    for(Entry<String, BlipData> entry : blipDatas.entrySet()) {
      blips.put(entry.getKey(), BlipImpl.deserialize(operationQueue, wavelet, entry.getValue()));
    }
