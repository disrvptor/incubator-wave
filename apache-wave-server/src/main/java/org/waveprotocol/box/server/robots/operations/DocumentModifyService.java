...
import com.google.wave.api.data.ElementMatcher;
import com.google.wave.api.data.Singleshot;
import com.google.wave.api.impl.DocumentModifyAction;
import com.google.wave.api.BundledAnnotation;
...
public class DocumentModifyService implements OperationService {
  ...
  private DocumentHitIterator getDocumentHitIterator(OperationRequest operation, ApiView view)
    ...
    if (range != null) {
      ...
      hitIterator = new Singleshot(range);
    } else if (index != null) {
      ...
      hitIterator = new Singleshot(new Range(index, index + 1));
    } else if (query != null) { // range and index are both null.
      // Use the query
      hitIterator = new ElementMatcher(
      ...
      hitIterator = new Singleshot(new Range(0, view.apiContents().length()));
    }
    return hitIterator;
  }
